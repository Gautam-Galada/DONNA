...
class AIDev:
    ...
    class SimpleNet(nn.Module):
        def __init__(self,hidden_size,input_size):
            ...
            self.linear_relu_stack=nn.Sequential(nn.Linear(input_size,hidden_size),nn.ReLU(),nn.Linear(hidden_size, 10))
        def forward(self,x):
            x=self.flatten(x)
    ...
    def modelinit(self, dataset_choice, data_dir=None):
        if dataset_choice=="mnist":
            ...
            transform=transforms.Compose([transforms.Resize(self.image_size),transforms.ToTensor(),transforms.Normalize((0.5,), (0.5,))])
    ...
    def datatype(self):return "images"
    ...
    def save_log(self, hyperparameters, final_loss, random_seed):
        log = {"hyperparameters": hyperparameters, "final_loss": final_loss, "random_seed": random_seed}
        with open('training_logs.json', 'a') as log_file:log_file.write(json.dumps(log) + '\n')

    def show_logs(self):
        if not os.path.exists('training_logs.json'):return []
